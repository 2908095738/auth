...
public class Add {
    ...
    private Boolean exec(AddRetailParams params) throws DatabaseException {
        ...
        // 更新库存药品数量
        if(isNeedUpdateStock(waitUpdateStockBatch)){
            updateStock(waitUpdateStockBatch);
        }
        if(!recordService.save(retailRecord)){
            throw new DatabaseException("零售记录入库失败");
        }
        fillRetailIdToDrugRecords(retailDrugRecords, retailRecord);

        if(!drugRecordService.saveBatch(retailDrugRecords))  {
            throw new DatabaseException("零售药品记录入库失败");
        }
    ...
    private void updateStock(List<StockBatch> waitUpdateStockBatch) throws DatabaseException {
        if(!stockService.update(waitUpdateStockBatch)) {
            throw new DatabaseException("库存数量更新失败");
        }
    }
