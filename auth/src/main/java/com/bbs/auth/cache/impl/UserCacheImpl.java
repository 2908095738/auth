...
public class UserCacheImpl implements UserCache {
    ...
    public void setUser(User user) {
        redis.set(RedisKeys.USER.key(user.getId()), toJsonPrettyStr(user), RandomUtil.randomInt(1, 5), TimeUnit.MINUTES);
    }
    ...
    private String getUserCacheKey(User user) {
        return RedisKeys.USER.key(user.getId());
    }
    ...
    private String getUserIDAndOpenIDMap(String openID) {
        return RedisKeys.USER_OPEN_ID_AND_ID_MAP.key(openID);
    }
    ...
    private String getUserIDAndPhoneMapKey(Long phone) {
        return RedisKeys.USER_PHONE_AND_ID_MAP.key(phone);
    }
    ...
    public User search(Long uid) throws InterruptedException, IllegalArgumentException {
        User user = redis.get(RedisKeys.USER.key(uid), User.class);

        if(nonNull(user)) {
            return user;

        } else {
            return RedisUtil.Redisson.lockExec(redisson.getSpinLock(RedisKeys.USER.LOCK.key(uid)), 500, 1000, MILLISECONDS, () ->
                    load(uid, 7, DAYS)
            );
        }
    }
    ...
    public User search(String email) throws InterruptedException, IllegalArgumentException {
        RLock lock = redisson.getSpinLock(RedisKeys.USER.LOCK.key(email));
    ...
    public VXUser searchByOpenID(String openid) throws IllegalArgumentException {
        return redissonUtil.lockExec(() -> {
            ...
            if(nonNull(uid)) {  // 微信已经绑定账号了
                String key = RedisKeys.USER.key(uid);
                ...
                redisson.getSpinLock(RedisKeys.USER.LOCK.key(openid)),
    ...
    private String searchUidFromRedis(String email) {
        return redis.getOpt(RedisKeys.USER_EMAIL_AND_ID_MAP.key(email)).get();
    }

    private Long searchUid(String openid) {
        String uidStr = redis.getOpt(RedisKeys.USER_OPEN_ID_AND_ID_MAP.key(openid)).get();
        return uidMapIsExist(uidStr) ? Long.valueOf(uidStr) : null;
    }

    private Long searchUid(Long phone) {
        String uidStr = redis.getOpt(RedisKeys.USER_PHONE_AND_ID_MAP.key(phone)).get();
        return uidMapIsExist(uidStr) ? Long.valueOf(uidStr) : null;
    }

    private void setUserAndEmailMapToRedis(User user) {
        redis.multiSet(new HashMap<String, String>() {{
            put(RedisKeys.USER_EMAIL_AND_ID_MAP.key(user.getEmail()), String.valueOf(user.getId()));
            put(RedisKeys.USER.key(user.getId()), toJsonPrettyStr(user));
        }});
    }
    ...
    public void updateByID(User user) throws BusinessException {
        ...
        RedisUtil.Redisson.lockExec(redisson.getSpinLock(RedisKeys.USER.LOCK.key(uid)), 500, 1000, MILLISECONDS, () -> {
    ...
    public User searchOrRegisterByPhone(Long phone) throws IllegalArgumentException {
        return redissonUtil.lockExec(() -> {
                    ...
                    User user;
                    if(nonNull(uid)) {
                        String userCacheKey = RedisKeys.USER.key(uid);
                ...
                redisson.getSpinLock(RedisKeys.USER_PHONE_AND_ID_MAP.LOCK.key(phone)),
                zkUtil.getIntForPath(ZookeeperNodePaths.LockConf.UserCache.WAIT),
                zkUtil.getIntForPath(ZookeeperNodePaths.LockConf.UserCache.LEASE),
                MILLISECONDS
        );
    ...
    private User load(Long uid, int timeout, TimeUnit unit) throws IllegalArgumentException {
        ...
        redis.set(RedisKeys.USER.key(uid), user, timeout, unit);
        return user;
    }
    ...
    private void load(User user, int timeout, TimeUnit unit) {
        redis.set(RedisKeys.USER.key(user.getId()), user, timeout, unit);
    }
